...
def tag_by_content(id):
    tags = db.session.query(Tag).filter(Tag.contents.any(Content.content_id == id)).all()
...
@app.route('/admin_content_page_new', methods=['GET', 'POST'])
@login_required
def admin_content_page_new():
    form = AdminContentPageNew(request.form)
    if request.method == 'POST':
        if form.validate_on_submit():
            new_content_page = Content(form.title.data, form.payload.data, form.author.data)
...
@app.route('/admin_content_page_edit/<int:id>', methods=['GET', 'POST'])
@login_required
...
@app.route('/content/', defaults={'id': ''}, methods=['GET'])
...
def signup():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            new_sign_up = User(form.username.data, form.email_address.data, form.password.data)
